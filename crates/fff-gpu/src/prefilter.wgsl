...
@group(0) @binding(8) var<storage, read> soa_ids: array<u32>;
...
// Paths are interleaved in 64-wide groups: word w of the path at lane l sits at
// soa_start + w * 64, so a workgroup's loads hit contiguous cache lines.
...
fn main(@builtin(workgroup_id) wg: vec3<u32>, @builtin(num_workgroups) nwg: vec3<u32>, @builtin(local_invocation_id) lid: vec3<u32>) {
    ...
    workgroupBarrier();
    let tid = (wg.y * nwg.x + wg.x) * 64u + lid.x;
    if (tid >= params.thread_count) { return; }
    let id = soa_ids[tid];
